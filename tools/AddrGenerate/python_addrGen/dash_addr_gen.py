...
def PubKeyToAddr(privKey, isTestnet = False):
    ...
    if isTestnet:
        return Base58CheckEncode(0x8c, ripemd160.digest())  #0x6F  p2pkh  testnet
    # return base58CheckEncode(0x05, ripemd160.digest())  #05  p2sh mainnet
    return Base58CheckEncode(0x4c, ripemd160.digest())  #00  p2pkh  mainnet
...
def PrivKeyToWIF(privKey, isTestnet = False):
    ...
    return Base58CheckEncode(0xcc, privKey.decode('hex') ) #0x80 主网
...
def PrivKeyToWIFCompress(privKey, isTestnet = False):
    ...
    return Base58CheckEncode(0xcc, privKey.decode('hex') + '\01') #0x80 主网
