...
def PubKeyToAddr(privKey, isTestnet = False):
    ...
    return Base58CheckEncode(0x30, ripemd160.digest())  #00  p2pkh  mainnet
...
def PrivKeyToWIF(privKey, isTestnet = False):
    ...
    return Base58CheckEncode(0xB0, privKey.decode('hex') ) #0x80 主网
...
def PrivKeyToWIFCompress(privKey, isTestnet = False):
    ...
    return Base58CheckEncode(0xB0, privKey.decode('hex') + '\01') #0x80 主网
