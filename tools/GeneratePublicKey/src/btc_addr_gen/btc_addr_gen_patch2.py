...
import base58
def ReversePrivKey(oldPrivKey, is_testnet ):
    rawPrivKey = base58.b58decode(oldPrivKey)
    # return PrivKeyToWIF(rawPrivKey[1:33].encode('hex'), True)
    # return PrivKeyToWIFCompress(rawPrivKey[1:33].encode('hex'), False)
    print(rawPrivKey[1:33].encode('hex'))
    return PrivKeyToWIF(rawPrivKey[1:33].encode('hex'), is_testnet)
...
def main():
    ...
    # print( ReversePrivKey('L53fss9LH88FSQHQWMYXWBo7qNwVTGSXteSXcrUkfDmsASNuNDcv'))
    print( ReversePrivKey('cSGCNnp3LxnRHaQnjrs3mRRX8wrSdeck5oDz51MhTyMx1mikrQKd', is_testnet=True))
