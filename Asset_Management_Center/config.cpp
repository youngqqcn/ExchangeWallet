...
#include "modules/encryptionModule/crypto_utils.h"


bool  g_bIsLAN = false; //使用是局域网, 通过  utils::Ping("192.168.10.1") 来检查即可
...
int StartOutsideExe(QString &strErrMsg)
{
    ...
    g_qstr_Xmr_OffSig_Port  = (g_bBlockChainMainnet) ? ("18089") : ("48089");
    if( true == g_bEnableMoneroWalletRpcExe && NULL != g_ptr_qprocess_MoneroWalletRpc )
    {
        //TODO: 钱包文件密码由管理员输入
        //QString strCmd = ".\\daemon\\xmr\\monero-wallet-rpc  --wallet-file=.\\daemon\\xmr\\manual_withdraw\\cold_wallet --confirm-external-bind --rpc-bind-ip 0.0.0.0 --rpc-bind-port 48089 --password 123456 --disable-rpc-login  --offline ";
        QString strCmd = ".\\daemon\\xmr\\monero-wallet-rpc  --wallet-file=.\\daemon\\xmr\\auto_withdraw\\cold_wallet --confirm-external-bind --rpc-bind-ip 0.0.0.0 --password 123456 --disable-rpc-login  --offline ";
...
//检查RSA的公私钥文件
static int CheckRsaPEMFile(QString &strErrMsg)
{
    std::vector<QString> vctPemPath;
    vctPemPath.push_back(QString(G_STR_A_PRIV_PEM_FILE_PATH));
    vctPemPath.push_back(QString(G_STR_B_PUB_PEM_FILE_PATH));

    for(auto strFilePath : vctPemPath)
    {
        QFile tmpFile(strFilePath);
        if(false == tmpFile.exists())
        {
            //TODO:弹出对话提示,配置文件不存在
            strErrMsg = QString("%1 not exists.").arg(strFilePath);
            qDebug() << "[file:" << __FILENAME__ << " line:" << __LINE__ << " ]" << strErrMsg ;
            return -1;
        }
    }
...
//检查RSA的私钥pem文件解密密码是否是对的(公钥PEM文件不加密)
bool IsValidPEMFilePassword(const QString &strPwd , QString &strErrMsg)
{
    std::string cstrPwd = strPwd.toStdString();
    std::string cstrPrivFilePath = QString(G_STR_A_PRIV_PEM_FILE_PATH).toStdString();
    unsigned char uszOutTmp[1024] = {0};
    memset(uszOutTmp, 0, sizeof(uszOutTmp));
    unsigned int uOutLen = 0;

    std::string cstrTestData = "test";
    int iRet = CryptoUtils::SHA256WithRSA_Sign_Ex(cstrPrivFilePath.c_str(), (unsigned char *)cstrTestData.c_str(),
                                                cstrTestData.length(), uszOutTmp, &uOutLen, (unsigned char *)cstrPwd.c_str());
    if( 0 == iRet /*|| iRet < -3 /*解密成功了, 但是签名失败*/)
    {
        return true;
    }

    strErrMsg = QString("解密文件%1失败, 解密密码错误!").arg(G_STR_A_PRIV_PEM_FILE_PATH);
    return false;
}
...
int InitEnvironment(QString &strErrMsg)
{
    ...
    //3.如果开启了充提币开关, 需要检查 pem文件是否存在
    if(g_bEnableCharge || g_bEnableWithdraw)
    {
        iRet = CheckRsaPEMFile(strErrMsg);
        if(0 != iRet)
        {
            strErrMsg = QString("您开启了\"enableCharge\" 或 \"enableWithdraw\"配置选项, 但是未检测到pem文件存在, 详细错误信息:") + strErrMsg;
            return iRet;
        }


        //4.只有开启了提币配置,并且私钥密码正确, 才启动外部服务进程
        if(g_bEnableWithdraw)
        {
