...
#author:yqq
#date:2019/4/9 19:05
#description:  up地址生成算法

import hashlib
import ecdsa
import os
import testbech32
from testbech32.segwit_addr import  bech32_encode
from testbech32.segwit_addr import  bech32_decode
from testbech32.segwit_addr import convertbits
...
def MyUSDPDecode2(hrp, addr):

    hrpgot , data =  bech32_decode(addr)
    print(hrpgot)

    decoded = convertbits(data, 5, 8, False)
    print("len(decode)", len(decoded))
    print("decoded:  ",  decoded)


    converted  = convertbits(decoded, 8, 5, True)
    print(len(converted))

    addr1 = bech32_encode(hrp, converted)
    print(addr1)



def MyUSDPEncode(hrp, witprog):
...
def  MyEncode():
    ...
    print("公钥:" + pubKey)
    print("len(pubkey)", len(pubKey.decode('hex')))
    ...
    print(len(r160data))
    rList = []
    for i in range(len(r160data)):
        rList.append(ord( r160data[i]))

    print(rList)
    addr = MyUSDPEncode("usdp", rList)
    print(addr)
    # MyUSDPDecode2('usdp', addr)
...
def main():
    ...
    # addr = MyUSDPEncode("usdp", pubKey.decode('hex'))
    # print("地址:" + addr)

    # s = 'usdp1p8hkj5fm2jwnzgqupqa5cjh9s3xhq5h78su6s5'
    # MyUSDPDecode('usdp', s)
    # MyUSDPDecode2('usdp', s)
    MyEncode()
